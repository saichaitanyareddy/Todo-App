...
export default function EditTaskPage() {
    ...
    const handleSubmission = async () => {
        ...
        const response = await fetch(`http://localhost:3200/tasks/${taskId}`, {
            method: "PUT",
            headers: {
    ...
    useEffect(() => {
        ...
        if (itemTitle) {
            setTaskTitle(itemTitle as string)
            setColor(itemColor as string);
            setTaskId(itemId as string);
        }
    }, [router]);
